...
def _show_palette(state):
    ...
    from tui_py.commands.ui.palette import ColorPalette
    import curses
...
def _load_theme(state, theme_name):
    """Load TDS theme."""
    from tui_py.ui.palette import ColorPalette, find_tds_theme
...
def _show_terminal_info():
    ...
    from tui_py.ui.palette import find_tds_theme
...
def _map_semantic_name(state, semantic_name, lane_id):
    ...
    from tui_py.ui.ui_utils import SEMANTIC_MAPPER
...
def _events_add(state, level, message):
    ...
    from tui_py.commands.content.clips import EventLevel
...
def _events_filter(state, filter_type, value):
    ...
    from tui_py.commands.content.clips import EventFilter, EventLevel
...
def _events_time_format(state, format):
    ...
    from tui_py.commands.content.clips import TimeFormat
...
def _video_load(state, video_path: str, lane_id: int = 5) -> str:
    ...
    # Load video
    try:
        from tui_py.commands.content.video_lane import VideoLane
        ...
        if not state.video_popup:
            from tui_py.commands.content.video_popup import VideoPopup
...
def _video_load_session(state, epoch: str) -> str:
    ...
    try:
        from tui_py.commands.content.video_lane import load_video_from_screentool_session
